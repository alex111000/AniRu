...
class LibriaCardViewHolder(
    ...
    fun bind(item: LibriaCard) {
        when (item.type) {
            ...
            is LibriaCard.Type.ProviderEpisode -> containerView.setMainImageDimensions(
                cardReleaseWidth,
                cardHeight
            )
        ...
        if (item.image.isBlank()) {
            // Metadata-only AnimeVost rows (catalog navigation / schedule) do not always
            // have posters. Keep the cards useful instead of rendering an empty tile.
            containerView.titleText = item.title
            containerView.contentText = item.description
            containerView.mainImageView?.setImageResource(R.mipmap.ic_banner)
        } else {
            containerView.titleText = null
            containerView.contentText = null
