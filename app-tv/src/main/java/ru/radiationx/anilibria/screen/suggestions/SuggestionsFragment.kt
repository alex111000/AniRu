...
class SuggestionsFragment : SearchSupportFragment(), SearchSupportFragment.SearchResultProvider {
    ...
    private fun getViewModel(rowId: Long): ViewModel? = when (rowId) {
        SuggestionsRowsViewModel.ANILIBRIA_RESULT_ROW_ID -> resultViewModel
        SuggestionsRowsViewModel.ANIMEVOST_RESULT_ROW_ID -> resultViewModel
        SuggestionsRowsViewModel.YUMMY_RESULT_ROW_ID -> resultViewModel
        SuggestionsRowsViewModel.SAMEBAND_RESULT_ROW_ID -> resultViewModel
    ...
    private fun createRowBy(
        ...
        SuggestionsRowsViewModel.ANILIBRIA_RESULT_ROW_ID,
        SuggestionsRowsViewModel.ANIMEVOST_RESULT_ROW_ID,
        SuggestionsRowsViewModel.YUMMY_RESULT_ROW_ID,
        SuggestionsRowsViewModel.SAMEBAND_RESULT_ROW_ID -> createHeaderRowBy(
            rowId,
    ...
    private fun createHeaderRowBy(
        ...
        val title = when (rowId) {
            SuggestionsRowsViewModel.ANILIBRIA_RESULT_ROW_ID -> "AniLibria"
            SuggestionsRowsViewModel.ANIMEVOST_RESULT_ROW_ID -> "AnimeVost"
            SuggestionsRowsViewModel.YUMMY_RESULT_ROW_ID -> "YummyAnime"
            SuggestionsRowsViewModel.SAMEBAND_RESULT_ROW_ID -> "SameBand"
            else -> "Результаты"
        }
        val row = ListRow(rowId, HeaderItem(title), cardsAdapter)
        when (rowId) {
            SuggestionsRowsViewModel.ANILIBRIA_RESULT_ROW_ID -> subscribeTo(viewModel.aniLibriaData) {
                cardsAdapter.setItems(it, CardDiffCallback)
            }
            SuggestionsRowsViewModel.ANIMEVOST_RESULT_ROW_ID -> subscribeTo(viewModel.animeVostData) {
                cardsAdapter.setItems(it, CardDiffCallback)
            }
            SuggestionsRowsViewModel.YUMMY_RESULT_ROW_ID -> subscribeTo(viewModel.yummyAnimeData) {
                cardsAdapter.setItems(it, CardDiffCallback)
            }
            SuggestionsRowsViewModel.SAMEBAND_RESULT_ROW_ID -> subscribeTo(viewModel.sameBandData) {
                cardsAdapter.setItems(it, CardDiffCallback)
            }
        }
